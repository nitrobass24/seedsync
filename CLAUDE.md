...
`CollectionService` (`src/angular/src/app/services/settings/collection.service.ts`) is the reference implementation — the shared base for `IntegrationsService` and `PathPairsService`: `createItem`/`updateItem`/`removeItem` use `tap`/`map` to update the subject inside the returned pipeline and `catchError` to map failures to a typed result, with no second subscribe.
