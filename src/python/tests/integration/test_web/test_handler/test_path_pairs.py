...
class TestPathPairsHandler(BaseTestWebApp):
    ...
    # ---------------------------------------------------------------
    def _add_pair(
        self, name="Movies", remote_path="/remote/movies", local_path="/local/movies", enabled=True, auto_queue=True
    ):
        """Add a pair directly to the config and return it."""
        pair = PathPair(
            name=name, remote_path=remote_path, local_path=local_path, enabled=enabled, auto_queue=auto_queue
        )
    ...
    def test_list_multiple(self):
        p1 = self._add_pair(name="Movies", remote_path="/r/movies", local_path="/l/movies")
        p2 = self._add_pair(name="TV", remote_path="/r/tv", local_path="/l/tv")
        p3 = self._add_pair(name="Music", remote_path="/r/music", local_path="/l/music")
        resp = self.test_app.get("/server/pathpairs")
        self.assertEqual(200, resp.status_int)
        pairs = json.loads(resp.text)
    ...
    def test_list_json_structure(self):
        ...
        self._add_pair(name="Test", remote_path="/r", local_path="/l", enabled=False, auto_queue=False)
        resp = self.test_app.get("/server/pathpairs")
        pairs = json.loads(resp.text)
        pair = pairs[0]
    ...
    def test_update_not_found(self):
        data = {"name": "Ghost"}
        resp = self._put_json("/server/pathpairs/nonexistent-id", data, expect_errors=True)
    ...
    def test_delete_not_found(self):
        resp = self.test_app.delete("/server/pathpairs/nonexistent-id", expect_errors=True)
        self.assertEqual(404, resp.status_int)
