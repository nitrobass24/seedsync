...
class TestControllerHandler(BaseTestWebApp):
    ...
    def test_cleanup_local(self):
        ...
        print(self.test_app.get("/server/command/cleanup_local/test1"))
        command = self.controller.queue_command.call_args[0][0]
        self.assertEqual(Controller.Command.Action.CLEANUP_LOCAL, command.action)
        self.assertEqual("test1", command.filename)

        uri = quote(quote("value/with/slashes", safe=""), safe="")
        print(self.test_app.get("/server/command/cleanup_local/" + uri))
        command = self.controller.queue_command.call_args[0][0]
        self.assertEqual(Controller.Command.Action.CLEANUP_LOCAL, command.action)
        self.assertEqual("value/with/slashes", command.filename)
