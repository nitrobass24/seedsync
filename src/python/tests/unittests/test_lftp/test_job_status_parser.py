...
class TestLftpJobStatusParser(unittest.TestCase):
    ...
    def test_chunk_without_range(self):
        """Test that \\chunk lines without a byte range (e.g., \\chunk 5077) don't crash."""
        output = (
            "jobs -v\n"
            "[2] pget -c ~/downloads/completed/SomeFile.mkv -o /data/lftpsync// \n"
            "    sftp://user:pass@host:22/home/user\n"
            "    `~/downloads/completed/SomeFile.mkv', got 1000 of 2000 (50%) \n"
            "  \\chunk 0-999\n"
            "    `~/downloads/completed/SomeFile.mkv' at 500 (25%) [Receiving data]\n"
            "  \\chunk 5077\n"
    ...
    def test_chunk_without_data_line(self):
        """Test that a \\chunk line at end of output (no data line) doesn't crash."""
        output = (
            "jobs -v\n"
            "[2] pget -c ~/downloads/completed/SomeFile.mkv -o /data/lftpsync// \n"
            "    sftp://user:pass@host:22/home/user\n"
    ...
    def test_queued_items(self):
        ...
        golden = [
            ...
            LftpJobStatus(
                job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
            LftpJobStatus(
                job_id=4, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
            LftpJobStatus(
                job_id=5, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
        ]
    ...
    def test_queued_items_with_quotes(self):
        ...
        golden = [
            LftpJobStatus(
                job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b s s", flags="-c"
            ),
            LftpJobStatus(
                job_id=2, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.QUEUED, name="a s s", flags="-c"
            ),
            LftpJobStatus(
                job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b s s", flags="-c"
            ),
            LftpJobStatus(
                job_id=4, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.QUEUED, name="a s s", flags="-c"
            ),
            LftpJobStatus(
                job_id=5, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
            LftpJobStatus(
                job_id=6, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.QUEUED, name="a", flags="-c"
            ),
            LftpJobStatus(
                job_id=7, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
            LftpJobStatus(
                job_id=8, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.QUEUED, name="a", flags="-c"
            ),
        ]
        self.assertEqual(len(golden), len(statuses))
        self.assertEqual(golden, statuses)
    ...
    def test_queue_and_jobs_1(self):
       ...
       \\transfer `ba/baa'
        ...
        golden_queue = [
            LftpJobStatus(
                job_id=1, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.QUEUED, name="c", flags="-c"
            ),
            LftpJobStatus(
                job_id=2, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
            LftpJobStatus(
                job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
        ...
        golden_job1.add_active_file_transfer_state("aa", LftpJobStatus.TransferState(None, None, None, 997, 22))
        ...
        golden_job2 = LftpJobStatus(
            job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="b", flags="-c"
        )
        golden_job2.total_transfer_state = LftpJobStatus.TransferState(35 * 1024, 394 * 1024, 8, 11059, None)
        golden_job2.add_active_file_transfer_state("bb", LftpJobStatus.TransferState(12333, 131072, 9, 3993, 30))
        golden_job2.add_active_file_transfer_state("ba/baa", LftpJobStatus.TransferState(9342, 131072, 7, 2969, None))
        golden_job2.add_active_file_transfer_state("ba/bab", LftpJobStatus.TransferState(13128, 131072, 10, 4096, 30))
    ...
    def test_queue_and_jobs_2(self):
        ...
        golden_queue = [
            LftpJobStatus(
                job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
            LftpJobStatus(
                job_id=2, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="b", flags="-c"
            ),
        ...
        golden_job2 = LftpJobStatus(
            job_id=2, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="b", flags="-c"
        )
        golden_job2.total_transfer_state = LftpJobStatus.TransferState(350 * 1024, 394 * 1024, 88, 11980, None)
        golden_job2.add_active_file_transfer_state("bb", LftpJobStatus.TransferState(124150, 131072, 94, 3993, 2))
        golden_job2.add_active_file_transfer_state("ba/baa", LftpJobStatus.TransferState(123531, 131072, 94, 3993, 2))
        golden_job2.add_active_file_transfer_state("ba/bab", LftpJobStatus.TransferState(101391, 131072, 77, 3993, 26))
        golden_jobs = [golden_job1, golden_job2]
        self.assertEqual(len(golden_queue) + len(golden_jobs), len(statuses))
        statuses_queue = [j for j in statuses if j.state == LftpJobStatus.State.QUEUED]
        self.assertEqual(golden_queue, statuses_queue)
    ...
    def test_queue_and_jobs_3(self):
        ...
        golden_queue = [
            ...
            LftpJobStatus(
                job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.QUEUED, name="e e", flags="-c"
            ),
        ]
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="a", flags="-c"
        ...
        golden_job2 = LftpJobStatus(
            job_id=2, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.RUNNING, name="d d", flags="-c"
        )
    ...
    def test_queue_and_jobs_4(self):
        r"""Queued items, parallel jobs running, '\mirror' line with 'Getting file list'"""
        output = """
        [0] queue (sftp://someone:@localhost) 
        sftp://someone:@localhost/home/someone
        ...
        golden_queue = [
            LftpJobStatus(
                job_id=1, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.QUEUED, name="rc", flags="-c"
            ),
        ]
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="ra", flags="-c"
        )
        ...
        golden_job2 = LftpJobStatus(
            job_id=2, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="rb", flags="-c"
        )
    ...
    def test_queue_and_jobs_5(self):
        r"""Queued items, parallel jobs running, '\mirror' line with 'cd'"""
        # noinspection PyPep8
        output = """
        [0] queue (sftp://someone:@localhost) 
        sftp://someone:@localhost/home/someone
    ...
    def test_queue_and_jobs_6(self):
        r"""Queued items, parallel jobs running, '\mirror' line with no units for local_size"""
        output = """
        ...
        golden_job1.add_active_file_transfer_state(
            "rab/raba", LftpJobStatus.TransferState(None, None, None, None, None)
        )
        golden_job1.add_active_file_transfer_state(
            "rab/rabb", LftpJobStatus.TransferState(None, None, None, None, None)
        )
        golden_job2 = LftpJobStatus(
            job_id=2, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="rb", flags="-c"
        )
    ...
    def test_jobs_2(self):
        ...
        golden_job1.add_active_file_transfer_state("aa", LftpJobStatus.TransferState(None, None, None, 90, 4 * 60))
        golden_jobs = [golden_job1]
        self.assertEqual(len(golden_jobs), len(statuses))
        statuses_jobs = [j for j in statuses if j.state == LftpJobStatus.State.RUNNING]
        self.assertEqual(golden_jobs, statuses_jobs)
    ...
    def test_jobs_3(self):
        ...
        statuses = parser.parse(output)
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.RUNNING, name="A.b.C.rar", flags="-c"
        )
    ...
    def test_jobs_quotes(self):
        ...
        statuses = parser.parse(output)
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="e e", flags="-c"
        )
        golden_job1.total_transfer_state = LftpJobStatus.TransferState(0, 132 * 1024, 0, None, None)
        golden_job1.add_active_file_transfer_state("e e a", LftpJobStatus.TransferState(None, None, None, 1003, 2 * 60))
        golden_job2 = LftpJobStatus(
            job_id=2, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.RUNNING, name="d d", flags="-c"
        )
    ...
    def test_jobs_mirror_empty(self):
        ...
        golden_job1.add_active_file_transfer_state("aa", LftpJobStatus.TransferState(None, None, None, None, None))
        golden_jobs = [golden_job1]
        self.assertEqual(len(golden_jobs), len(statuses))
        statuses_jobs = [j for j in statuses if j.state == LftpJobStatus.State.RUNNING]
        self.assertEqual(golden_jobs, statuses_jobs)
    ...
    def test_jobs_almost_done(self):
        ...
        statuses = parser.parse(output)
        golden_job1 = LftpJobStatus(
            job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="c'c'c'c", flags="-c"
        )
    ...
    def test_jobs_missing_pget_data_line(self):
        ...
        statuses = parser.parse(output)
        golden_job1 = LftpJobStatus(
            job_id=3, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="c -o c", flags="-c"
        )
        golden_job1.total_transfer_state = LftpJobStatus.TransferState(None, None, None, None, None)
        golden_job2 = LftpJobStatus(
            job_id=4, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.RUNNING, name="d -o d.txt", flags="-c"
        )
        golden_job2.total_transfer_state = LftpJobStatus.TransferState(None, None, None, None, None)
        golden_jobs = [golden_job1, golden_job2]
        self.assertEqual(len(golden_jobs), len(statuses))
    ...
    def test_jobs_special_char_1(self):
        ...
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="aaa'aaa", flags="-c"
        )
    ...
    def test_jobs_special_char_4(self):
        ...
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="a -o a", flags="-c"
        )
        golden_job1.total_transfer_state = LftpJobStatus.TransferState(60, 128, 46, 16, None)
        golden_job1.add_active_file_transfer_state("a -o a.txt", LftpJobStatus.TransferState(None, None, None, 16, 4))
        golden_job2 = LftpJobStatus(
            job_id=2, job_type=LftpJobStatus.Type.PGET, state=LftpJobStatus.State.RUNNING, name="b -o b.txt", flags="-c"
        )
        golden_job2.total_transfer_state = LftpJobStatus.TransferState(None, None, None, 26, 1)
        golden_jobs = [golden_job1, golden_job2]
        self.assertEqual(len(golden_queue) + len(golden_jobs), len(statuses))
    ...
    def test_jobs_chmod(self):
        ...
        golden_job2.add_active_file_transfer_state(
            "star.battle.movie.720p.r07", LftpJobStatus.TransferState(44628032, 50000000, 89, 1153433, 5)
        )
        golden_job2.add_active_file_transfer_state(
            "star.battle.movie.720p.r08", LftpJobStatus.TransferState(15237120, 50000000, 30, 2139095, None)
        )
        golden_job2.add_active_file_transfer_state(
            "star.battle.movie.720p.r09", LftpJobStatus.TransferState(21692416, 50000000, 43, 4246732, 16)
        )
        golden_job2.add_active_file_transfer_state(
            "star.battle.movie.720p.r10", LftpJobStatus.TransferState(33930272, 50000000, 67, 5305794, 6)
        )
    ...
    def test_jobs_chmod_two_liner(self):
        ...
        golden_job1.add_active_file_transfer_state(
            "Space.Trek.S08E05/space.trek.s08e05.r06", LftpJobStatus.TransferState(None, None, None, None, None)
        )
        golden_job1.add_active_file_transfer_state(
            "Space.Trek.S08E06/space.trek.s08e06.nfo", LftpJobStatus.TransferState(None, None, None, None, None)
        )
        golden_job1.add_active_file_transfer_state(
            "Space.Trek.S08E07/space.trek.s08e07.nfo", LftpJobStatus.TransferState(None, None, None, None, None)
        )
        self.assertEqual(1, len(statuses))
        self.assertEqual(golden_job1, statuses[0])
    ...
    def test_jobs_chmod_2(self):
        ...
        golden_job1.add_active_file_transfer_state(
            "Space.Trek.mkv", LftpJobStatus.TransferState(7700480, 425302375, 1, 1059061, 420)
        )
    ...
    def test_jobs_removes_multiple_jobs_lines(self):
        ...
        statuses = parser.parse(output)
        golden_job1 = LftpJobStatus(
            job_id=1, job_type=LftpJobStatus.Type.MIRROR, state=LftpJobStatus.State.RUNNING, name="movie", flags="-c"
        )
